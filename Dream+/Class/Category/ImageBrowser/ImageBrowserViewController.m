...
-(void)loadPhote:(NSInteger)index{
    ...
    if (![currentPhotoView isKindOfClass:[PhotoView class]]) {
        ...
        if ([[self.imagesArray firstObject] isKindOfClass:[NSString class]]){
            PhotoView *photoV = [[PhotoView alloc] initWithFrame:frame withPhotoUrl:[self.imagesArray objectAtIndex:index]withDate:self.createDate];
            //photoV.createDate=self.createDate;
        ...
        } else if ([[self.imagesArray firstObject] isKindOfClass:[UIImage class]]){
            PhotoView *photoV = [[PhotoView alloc] initWithFrame:frame withImage:[self.imagesArray objectAtIndex:index]withDate:self.createDate];
            //photoV.createDate=self.createDate;
            photoV.delegate = self;
            [self.scrollView insertSubview:photoV atIndex:0];
            [_subViewArray replaceObjectAtIndex:index withObject:photoV];
...
#pragma mark - 生成显示窗口
+(void)show:(UIViewController *)handleVC type:(PhotoBroswerVCType)type index:(NSUInteger)index andCreateDate:(NSString *)createDate imagesBlock:(NSArray *(^)())imagesBlock{
    ...
    imgBrowserVC.createDate=createDate;
...
-(UIPageControl *)pageControl{
    if (_pageControl==nil) {
        ...
        _pageControl.pageIndicatorTintColor = BMColor(47, 150, 86);
